...
function has_valid_fields(passport)
  all(((k, v),) -> !haskey(field_validators, k) || field_validators[k](v), pairs(passport))
end

# Convert input to an array of Dicts, where each dict is a passport.
input = split(read("day4-input.txt", String), r"\n{2,}") .|> split .|> e -> Dict(split.(e, ":"))

num_valid_part1 = count(has_required_fields, input)
print("Part 1: Num valid passports = $(num_valid_part1)\n")

num_valid_part2 = count(p -> has_required_fields(p) && has_valid_fields(p), input)
